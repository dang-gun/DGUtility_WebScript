...
export default class DG_Popup2 {
    constructor(jsonShowDefaultOption) {
         ...
         * @returns {void}
         */
        this.TitleMouseDownEvent = (event) => {
            ...
            const SelectDiv = Parent.parentElement.parentElement;
            // 좌표가 수정될 상위 DOM을 찾는다.
            // this.CurrentSelectDiv = event.target.parentElement.parentElement;
            this.CurrentSelectDiv = SelectDiv;
            this.CurrentSelectIndex = this.CurrentSelectDiv.getAttribute("data-index");
            // 마우스 상태 변경
            this.MouseState = 1;
            // 마우스 다운 계산값
         ...
         * @returns {void}
         */
        this.TitleMouseMoveEvent = (event) => {
         ...
         * @returns {void}
         */
        this.TitleMouseUpEvent = (event) => {
            // 마우스 상태 변경
            this.MouseState = 2;
        };
        console.log('DG_Popup2 start');
        // 기본 옵션을 사용자 옵션으로 덮어쓴다.
        this.ShowDefaultOption
            = Object.assign(this.ShowDefaultOption, jsonShowDefaultOption);
        const PopupTitleElement = document.querySelector(".DG_PopupTitle");
    ...
    Show(jsonOption) {
        ...
        // 기본 옵션을 사용자 옵션으로 덮어쓴다.
        const NewJsonOption = Object.assign({}, jsonShowDefaultOption, jsonOption);
        // 고유키 증가
        let nPopupIndex = ++this.PopupIndex;
        // 고유키 확인
        ...
        // 컨텐츠용 div를 생성한다.
        const PopupContentElement = this.CreatePopupContent(NewJsonOption, nPopupIndex, nZIndex);
        ...
        // 새롭게 추가한 팝업 개체를 찾는다.
        const NewPopupParentElement = document.querySelector("#divDG_PopupParent" + nPopupIndex);
        const NewPopupOverlayElement = document.querySelector("#divDG_PopupOverlay" + nPopupIndex);
        const NewPopupContentElement = document.querySelector("#divDG_Popup" + nPopupIndex);
        ...
        if ("center" === NewJsonOption.top) {
            // 센터일 경우
            // 중앙값을 계산한다.
            const nTopCenter = (window.innerHeight / 2)
        ...
        if ("center" === NewJsonOption.left) {
            // 센터일 경우
            // 중앙값을 계산한다.
            const nLeftCenter = (window.innerWidth / 2)
     ...
     */
    CreatePopupParent(NewJsonOption, nPopupIndex) {
        // 부모용 div를 생성한다.
     ...
     */
    CreatePopupOverlay(NewJsonOption, nPopupIndex, nZIndex) {
        // Overlay를 생성한다.
     ...
     */
    CreatePopupContent(NewJsonOption, nPopupIndex, nZIndex) {
        // 컨텐츠용 div를 생성한다.
